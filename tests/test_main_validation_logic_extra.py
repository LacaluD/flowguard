...
def test_run_yq_timeout_returns_one(
    ...
    monkeypatch.setattr(pipeline, "count_timeout", lambda **_: 5)

    assert pipeline.run_yq(yml_file, ".", "base", Path("yq")) == 1
...
def test_build_job_scoped_validation_yaml_returns_none_on_parse_error(
    ...
    monkeypatch.setattr(
        pipeline.yaml,
        "safe_load",
        lambda *_: (_ for _ in ()).throw(pipeline.yaml.YAMLError("bad")),
    )
...
def test_build_job_scoped_validation_yaml_success_with_quoted_on_key(
    ...
    result = pipeline._build_job_scoped_validation_yaml(
        cfg_file=yml_file, job_name="build"
    )
...
def test_regular_validation_rejects_multiple_cfg_files(
    ...
    result = pipeline.regular_validation(
...
""".strip() + "\n",
    ...
    monkeypatch.setattr(validator, "validate_config", lambda **_: 1)
    monkeypatch.setattr(
        pipeline,
        "build_dot_scheme",
...
""".strip() + "\n",
    ...
    def fake_validate_config(*, yml_path: Path, **kwargs):
        ...
        captured["run_optional"] = validator.run_optional
        return 0
...
def test_validate_config_supports_json_by_converting_to_yaml_for_yq(
    ...
    def fake_pool(*, fpath: Path, fending: str):
        seen["pool"] = fpath
        assert fpath.suffix == ".yml"
        assert fending == ".json"
        return 0
    ...
    result = validator.validate_config(json_file)
...
def test_validate_config_supports_toml_by_converting_to_yaml_for_yq(
    ...
    result = validator.validate_config(toml_file)
