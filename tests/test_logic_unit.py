...
def test_find_yaml_files_with_yaml_file_returns_single_path(tmp_path: Path) -> None:
    ...
    result = pipeline._collect_yaml_files(yml_file, excluded_dirs=[])
...
def test_find_yaml_files_with_non_yaml_file_returns_empty_list(tmp_path: Path) -> None:
    ...
    result = pipeline._collect_yaml_files(txt_file, excluded_dirs=[])
...
def test_find_yaml_files_with_directory_collects_yml_and_yaml(tmp_path: Path) -> None:
    ...
    toml_file.write_text('name = "toml"\n', encoding="utf-8")
...
def test_check_for_empty_file_returns_one_for_empty_file(tmp_path: Path) -> None:
    ...
    assert pipeline.check_for_empty_file(empty) == 1
...
def test_check_for_empty_file_returns_zero_for_non_empty_file(tmp_path: Path) -> None:
    ...
    assert pipeline.check_for_empty_file(non_empty) == 0
...
def test_check_for_empty_file_returns_one_on_os_error(
    ...
    assert pipeline.check_for_empty_file(yml_file) == 1
...
def test_check_for_deprecated_keys_returns_zero_for_empty_content(
    ...
    result = pipeline.check_for_deprecated_keys(tmp_path / "wf.yml", "")

    assert result == 0


def test_normalize_action_ref_returns_none_without_at_symbol() -> None:
    assert pipeline._normalize_action_ref("actions/checkout") is None


def test_normalize_action_ref_trims_quotes_and_v_prefix() -> None:
    assert pipeline._normalize_action_ref("'actions/checkout@v3'") == (
        "actions/checkout",
        "3",
    )


def test_normalize_action_ref_returns_none_for_blank_parts() -> None:
    assert pipeline._normalize_action_ref("@v3") is None
    assert pipeline._normalize_action_ref("actions/checkout@v") is None


def test_check_for_deprecated_keys_skips_invalid_deprecated_entries(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr(pipeline, "DEPRECATED_ACTIONS", ["invalid"])

    result = pipeline.check_for_deprecated_keys(
        tmp_path / "wf.yml", "uses: actions/checkout@v3"
    )
...
def test_check_indentation_detects_mixed_indent_error(tmp_path: Path) -> None:
    ...
    assert pipeline.check_indentation(yml_file) == 1
...
def test_run_yq_in_threadpool_includes_optional_checks_when_enabled(
    ...
    monkeypatch.setattr(pipeline, "EXTENDED_CHECKS", [".name"])
    monkeypatch.setattr(pipeline, "OPTIONAL_CHECKS", [".jobs[].needs"])
    ...
    result = validator.run_yq_in_threadpool(
        fpath=yml_file,
        fending=".json",
    )
...
def test_build_dot_scheme_returns_none_for_empty_input() -> None:
    result = pipeline.build_dot_scheme([], Path("yml2dot"))
...
def test_build_dot_scheme_returns_none_on_dot_failure(
    ...
    monkeypatch.setattr(pipeline.subprocess, "run", raise_dot_error)

    result = pipeline.build_dot_scheme([yml_file], Path("yml2dot"))
...
def test_build_dot_scheme_returns_last_png_path_on_success(
    ...
    result = pipeline.build_dot_scheme([first, second], Path("yml2dot"))

    assert result == second.with_suffix(".svg")
