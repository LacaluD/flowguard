...
def test_validate_single_yaml_handles_os_error(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    f = tmp_path / "wf.yml"
...
def test_validate_against_schema_returns_one_when_schema_file_missing(tmp_path: Path) -> None:
    yaml_file = tmp_path / "wf.yml"
    yaml_file.write_text("name: CI\n", encoding="utf-8")
...
def test_validate_against_schema_handles_unexpected_exception(
    ...
    monkeypatch.setattr(validation_by_schema,
                        "_collect_yaml_files", lambda _: [yaml_file])
    monkeypatch.setattr(validation_by_schema, "_load_schema",
                        lambda _: {"type": "object"})
    monkeypatch.setattr(validation_by_schema,
    ...
    monkeypatch.setattr(validation_by_schema,
                        "_validate_single_yaml", raise_unexpected)
...
def test_validate_custom_pipeline_returns_one_when_schema_validation_fails(
    ...
    monkeypatch.setattr(validation_by_schema,
                        "validate_against_schema", lambda **_: 1)
...
def test_validate_custom_pipeline_returns_zero_on_success(
    ...
    monkeypatch.setattr(validation_by_schema,
                        "_collect_yaml_files", lambda _: [yml_file])
    monkeypatch.setattr(validation_by_schema, "build_dot_scheme",
