...
def _build_selected_job_yaml(cfg_file: Path, job_name: str) -> Path | None:
    ...
    try:
        ...
        log_exception_short(
            logger, exc, prefix=f"Failed to extract job from {cfg_file}", level="error"
        )
        return None
...
def _build_yaml_from_supported_config(cfg_file: Path) -> Path | None:
    ...
    try:
        ...
        log_exception_short(
            logger, exc, prefix=f"Failed to parse {cfg_file}: {exc}", level="error"
        )
        return None
    tmp = tempfile.NamedTemporaryFile(
        mode="w", suffix=".yml", encoding="utf-8", delete=False
    ...
    for f in cfg_files:
        ...
        try:
            ...
            log_exception_short(
                logger,
                exc,
                prefix=f"diagram generation timed out after {timeout}s on file: {f}",
                level="error",
            )
            return None
